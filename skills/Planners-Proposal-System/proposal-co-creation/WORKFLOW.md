...
### 3. Storyline 是有信息量的判断链

Storyline 描述客户必须依次接受的判断。每个节点是一句**有信息量的逻辑陈述句**，让人看出因为什么、所以什么：

- 携带具体事实与推论，不是主题标签，也不是认知动作（「看清 X」「先立口径」都不是节点）；
- 用陈述语气表达因果或推断关系；不用反问，不用「你以为／其实」这类修辞反转，不用破折号，不用强调性引号；
- 层层递进：后一个节点以前一个节点已经确立的判断为前提。

方法与口径、数据边界、样本说明不构成节点，它们是页内边界条或附录。

一个判断节点可以展开为数页，因为事实、比较、案例、模型和行动含义可能需要分别证明。页数不由节点数决定，但**每一页都要能归属到某一句判断**——说不清自己在证哪一句的页不该存在。
